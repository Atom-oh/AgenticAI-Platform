...
"In-Region" 표현 사용을 금지한다 — "VPC 프라이빗 서브넷"으로 표기한다(UI 문자열·화면 라벨 포함).

SPEC v2 추가 규칙:
- **§1-1 정합성 감사가 코드 수정보다 먼저다** — 명세 항목 vs 데모 현황 표를 만들고,
  "명세에 없는 주장을 데모가 하지 않게 한다". 큰 변경 전에 이 감사를 갱신한다.
- **§10 사용 금지: AWS CodeCommit·CodePipeline.** CI/CD와 자산 원본(컴포넌트 계약·화면 스펙·Skills)은
  **기존 GitLab을 전제로 표기**한다(§7 매핑) — 단, 데모 범위에 GitLab 구축은 없으므로 화면에서는
  §11 배지 원칙대로 "데모: Registry 버전 관리 / 운영: GitLab 원본"으로 사실대로 표기한다.
- **§14 확인 질문은 v2에서 다시 열렸다** — 에이전트 SDK·AgentCore 사용 범위·벡터 저장소는
  v1 답변을 자동 승계하지 말고, 사용자 확정을 SPEC(§16 구현 메모)에 기록한 뒤 진행한다.
