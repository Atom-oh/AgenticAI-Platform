...
**Round-state content permission** (PR #26 review round 3, `AGENTCORE_CONTRACT.md:443`). The screen detail route and **every source-bearing response** call the same shared check: every `GET /runs/{id}/blob` chunk whose kind carries round source (`kind=source` and `kind=candidate`, whose JSON holds the generated source map per `react_generation.py:157-159`; any future source-bearing kind is added to one shared `SOURCE_BEARING_KINDS` set that the handler consults, and unknown kinds fail closed), for design and legacy runs alike. They call as the B0 sharing H1 adapter (`round_content_allowed(role, round)`): a draft/failed/needs-changes round's source is served only to designer/developer/owner; reviewable, approved and released rounds to current project readers. This also closes the existing gap where the legacy run-blob handler serves failed-round source to any `read` member; non-design legacy runs get the same rule. Tests: a planner is denied failed/draft source on every route and kind (screen detail omits it; `kind=source` and `kind=candidate` → 404, on the first and later chunks) (screen detail omits it, blob → 404), a designer receives it, and a planner receives a reviewable round's source.
