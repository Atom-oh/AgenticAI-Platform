...
라이브 React 모델·실제 승인·AWS 릴리스·비공개 아카이브 다운로드를 확인했다.
...
고객 React 패키지는 미제공이며 제공 후 별도 연결·검증 대상이다. 현재는 **플랫폼 기본 패키지**로 표시한다.
...
라이브 생성 확인은 기준안+변형 2개와 creative 1개다. 변형 5개 상한은 자동 테스트 근거와 구분하며,
별도 결과가 기록되지 않은 라이브 규칙 제안까지 완료로 확장하지 않는다.
...
- [x] 선택한 라이브 Astra 기준안·Fable 시안을 AWS의 실제 트랜잭션으로 승인.
- [x] 두 승인본 모두 AI 호출 없는 같은 소스의 AWS 재빌드·실제 브라우저·승인 화면 대비 2% 비교 통과.
- [x] 비공개 S3 소스·dist ZIP 저장과 인증 다운로드 해시 일치 확인.

| 검증한 릴리스 | 소스 ZIP | dist ZIP |
|---|---:|---:|
...
**고객 Git 원격 대상은 미설정**이다. 대상·권한·비밀 참조 제공 후의 실제 원격 쓰기는 별도 검증 범위다.
...
- [x] 프런트엔드 게시 후 실제 관리자 계정의 공동 프로젝트·React 미리보기·릴리스 상태와 세 모니터 폭을 확인. 역할별 접근은 별도 자동 테스트로 검증.
...
- [x] 실제 승인·AWS 릴리스 통과·아카이브 크기와 해시 확인, 원본 이미지 비교와 승인 화면 비교의 차이를 기록.
- [ ] 승인된 공개 소스 범위로 PR 생성.
- [ ] 최종 PR head의 독립 코드·보안 검토, Critical/Major 수정 후 병합 여부 판단.
게시 후 UI 확인과 상태 갱신은 §6의 체크 항목으로 관리한다.
