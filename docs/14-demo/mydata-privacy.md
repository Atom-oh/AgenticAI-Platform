...
기준: 2026-09-12 PR3 구현과 2026-09-13 코드 대조. 루트 `SPEC.md` §4-3·§11-1을 따른다. 구현·테스트와 실제 사설 경로 배포·모델 정확도 검증은 구분한다.
...
질문은 입력 Guardrails보다 먼저 처리하며, 조회·계산으로 만든 설명 자료도 설명 모델에 전달하기 전에 다시 검사한다. 정형 설명 자료는 신뢰된 내부 생산자가 토큰화하고 독립 규칙·Bedrock Guardrails가 검사한다. 두 번째 sLLM 재작성은 하지 않으며 `privacy_payload`는 `modelInvoked: false`로 표시한다.

코드는 설명을 버퍼링하고 출력 Guardrails 처리 후 텍스트를 전달한다. `/s2/finalize`의 수치 검증·재식별과 Semantic 비교는 그 뒤에 수행한다. 따라서 위 흐름도를 모든 수치 오류가 화면 표시 전에 차단된다는 보장으로 해석하지 않는다.
...
현행 사설 게이트웨이는 독립 NER를 **미구성**(`not-configured`)으로 반환한다. 이 영수증을 별도 NER 모델 실행으로 해석하지 않으며, 이후 Bedrock Guardrails의 독립 검사와도 구분한다.
...
개인정보 처리 경로는 인증 API → IAM 전용 Lambda 중계 → 내부 로드밸런서 → EKS CPU 처리기 → 사설 모델 서비스로 구현돼 있다. 질문 원문은 API와 중계를 지나므로 사설 모델 배치를 전체 사용자 데이터가 API 밖으로 나오지 않는다는 뜻으로 설명하지 않는다.
`platform/infra/lib/privacy-stack.ts`는 기존 EKS VPC를 재사용하며 데이터 플레인 VPC와 별도다. GPU·VPC 피어링은 추가하지 않고, 배포된 중계 ARN·게이트웨이 이미지·대상 연결은 별도로 확인한다.
공용 모델 엔드포인트나 Lambda Function URL을 만들지 않는다.
로드밸런서 컨트롤러·NLB 대상과 주소·네트워크 정책의 실제 집행을 확인한 뒤 연결 완료로 판단한다. `/health`는 게이트웨이 상태이며 실제 모델 준비 상태는 모델 목록 조회와 별도 호출로 확인한다.
...
구현 근거: `platform/api/handlers/s2.py`, `platform/api/common/privacy.py`, `platform/api/common/pii.py`, `platform/privacy/gateway/`, `platform/privacy/relay/handler.py`, `platform/privacy/deploy/`. 배포 전제와 운영 확인은 `platform/infra/README-privacy.md`를 따른다.
