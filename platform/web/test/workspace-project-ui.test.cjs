...
test('project scope, collaboration, guided baselines and real release metadata remain isolated', { timeout: 120_000 }, async t => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '화면 만들기' }).click();
    ...
    if (process.env.WORKSPACE_QA_DIR) {
      ...
      await page.screenshot({ path: path.join(process.env.WORKSPACE_QA_DIR, 'handoff-approved.png'), fullPage: true });
    }
    ...
    await page.getByText('게시 기준 확인·새 버전 작성', { exact: true }).click();
    ...
    await page.waitForFunction(() => document.activeElement === document.querySelector('.ws-stage-heading h2'));
    await page.getByRole('button', { name: '화면 만들기', exact: true }).click();
    assert.equal(await page.getByLabel('협업할 화면', { exact: false }).inputValue(), 'review');
    ...
    await page.getByRole('button', { name: '업무 정의', exact: true }).click();
    if (!(await page.getByLabel('조건 1', { exact: true }).isVisible()))
      await page.getByText('게시 기준 확인·새 버전 작성', { exact: true }).click();
    ...
    await page.evaluate(() => { location.hash = '#/studio?projectId=b&step=handoff&runId=b-approved&round=4'; });
    await page.getByText('연결된 라운드를 찾지 못했습니다. 다른 승인본으로 자동 전환하지 않습니다.', { exact: true }).waitFor();
    assert.equal(await page.getByLabel('전달할 라운드', { exact: true }).inputValue(), '4');
    assert.equal(await page.getByRole('button', { name: '승인 소스 재빌드·릴리스 검사' }).count(), 0);
    assert.equal(await page.getByRole('button', { name: '기능 브랜치로 내보내기' }).isDisabled(), true);
    assert(calls.some(call => call.target === '/runs/b-approved' && call.scope === 'b'));
    await page.evaluate(() => { location.hash = '#/studio?projectId=revoked&step=handoff&runId=b-approved'; });
    await page.getByText('이 프로젝트의 현재 참여 권한을 확인할 수 없습니다.', { exact: true }).waitFor();
    assert.equal(calls.some(call => call.target === '/runs/b-approved' && call.scope === 'revoked'), false);
