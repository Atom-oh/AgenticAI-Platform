...
const VARIANTS = { balanced: '기본 구성', dense: '정보를 한눈에', emphasis: '핵심 강조', flow: '진행 흐름 강조' } as const;

export default function RunsPanel({ config, assets, contracts, runs, refresh, preferredContract, editing }: {
  ...
  }, [contractId, currentContractVersion]);
  useEffect(() => {
  ...
  const evidenceKey = `${runId}:${roundNumber}:${selectedRound?.artifactSha256 || ''}`;
  ...
  const canGenerate = canUseContract && config.models.some(option => option.id === model);
  ...
  const generate = (count: 1 | 3) => {
    ...
    const entries = (count === 3 ? ['balanced', 'dense', 'emphasis'] as const : [variant]).map(value => {
      const id = crypto.randomUUID();
      return { id, label: VARIANTS[value], payload: {
        requestId: id, contractId: contract.id, contractVersion: contract.version, mode: 'generate', model, maxRounds: rounds, variant: value,
        ...(reference ? { referenceAssetId: reference, referencePage, visualTolerance: tolerance } : {}),
      } };
    });
    setAttempts(previous => [...entries, ...previous]); setRunId(''); setRoundNumber(0); setInstruction(''); void submit(entries);
  };
  const verifyOriginal = () => {
    if (!canUseContract || !contract || !selectedHtmlId || sendingRef.current || approving) return;
    const id = crypto.randomUUID();
  ...
  const revise = () => {
    if (!run || !selectedRound?.hasHtml || !instruction.trim() || sendingRef.current || !config.models.some(option => option.id === model)) return;
    const id = crypto.randomUUID();
    const entry: Attempt = { id, label: `라운드 ${selectedRound.number} 수정`, payload: {
      requestId: id, contractId: run.contractId, contractVersion: run.contractVersion, mode: 'generate', model, maxRounds: rounds,
  ...
  const approve = async () => {
    if (!run || !selectedRound || !approvedCheck || !roundApprovable(selectedRound) || approving || sending) return;
  ...
  const selectRun = (id: string) => { setRunId(id); setRoundNumber(0); setInstruction(''); setApprovedCheck(false); };
  ...
  return <div className="ws-runs">
    <section className="ws-section">
      <div className="ws-section-heading"><div><h2>승인한 기준으로 HTML 검사·시안 만들기</h2><p>반입한 HTML을 검사하거나 AI로 시안을 만듭니다. 동작 검증, 시작 화면 기준 비교, 사람의 승인은 별도이며 실제 금융 API는 호출하지 않습니다.</p></div></div>
      <div className="ws-generation-fields">
        <label className="ws-field">사용할 규칙<select value={contractId} onChange={event => setContractId(event.target.value)} disabled={sending}>
        ...
        <label className="ws-field">화면 방향<select value={variant} onChange={event => setVariant(event.target.value as keyof typeof VARIANTS)} disabled={sending}>
          {Object.entries(VARIANTS).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
      </div>
      ...
      <div className="ws-actions"><button className="ws-primary" onClick={() => generate(1)} disabled={!canGenerate || sending || approving}>시안 1개 만들기</button>
        <button onClick={() => generate(3)} disabled={!canGenerate || sending || approving}>방향이 다른 3개 만들기</button>
        {htmlSources.length > 0 && <button onClick={verifyOriginal} disabled={!canUseContract || !selectedHtmlId || sending || approving}>반입 HTML 검사</button>}</div>
      ...
      <section className="ws-section ws-result">
        ...
        {!run ? <div className="ws-empty">{runId ? '시안 기록을 불러오고 있습니다…' : '확인할 시안을 선택하세요.'}</div> : <>
          ...
          <div className="ws-rounds" aria-label="시안 라운드">{(run.rounds || []).map(round => <button key={round.number}
            ...
            라운드 {round.number} · {round.passed ? '필수 검사 통과' : '확인 필요'}{round.number === run.bestRound ? ' · 최종 선택' : ''}</button>)}</div>
          {selectedRound ? <>
            ...
            <div className="ws-result-columns">
              ...
              <div className="ws-evidence"><RoundEvidence key={evidenceKey} run={run} round={selectedRound}
                ...
                <button className="ws-primary" onClick={revise} disabled={sending || approving || selectedRound.hasHtml !== true || !instruction.trim() ||
            ...
            <div className="ws-approval"><label className="ws-check"><input type="checkbox" checked={approvedCheck}
              disabled={!roundApprovable(selectedRound) || approving} onChange={event => setApprovedCheck(event.target.checked)} />
              이 라운드의 동작·시작 화면 비교 범위와 근거를 확인했습니다</label>
              <button onClick={() => void approve()} disabled={!roundApprovable(selectedRound) || !approvedCheck || approving || sending}>
...
function RoundEvidence({ run, round, onLoaded }: { run: Run; round: Round; onLoaded: (report: Record<string, unknown>) => void }) {
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [run.id, round.number, round.hasReport, retry]);
