      ...
      <div className="flex items-center gap-2 mb-4">
        {([['gallery', '갤러리'], ['assets', '디자인 자산'], ['generate', '생성']] as const).map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            className={`chip ${tab === id ? 'text-teal-700 border-teal-400' : 'text-slate-400 hover:border-slate-400'}`}>{label}</button>
        ))}
...
function Gallery({ canWrite }: { canWrite: boolean }) {
  ...
  const color = (s: string) => s === '승인됨' ? 'text-emerald-600' : s === '반려' ? 'text-[#E90061]' : 'text-amber-700';
  return (
        ...
        <div key={d.id} className="panel overflow-hidden">
          ...
          <div className="p-3">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <a className="text-xs text-[#008485] ml-auto" href={d.url} target="_blank" rel="noopener">원본 ↗</a>
            </div>
            {canWrite && d.status === '검토중' && (
              <div className="flex gap-2 mt-2">
                <button disabled={busy === d.id} onClick={() => decide(d.id, 'approve')}
                  className="chip text-emerald-700 hover:border-emerald-400 flex-1 justify-center">승인 → few-shot 학습</button>
                <button disabled={busy === d.id} onClick={() => decide(d.id, 'reject')}
                  className="chip text-[#E90061] hover:border-rose-600 flex-1 justify-center">반려</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="panel overflow-hidden self-start">
        <table className="w-full text-sm">
          ...
          <tbody>{assets.map((a, i) => (
            <tr key={i} onClick={() => open(a)} className="border-b border-slate-100 cursor-pointer hover:bg-slate-100">
      ...
      <div className="space-y-4">
        {sel && (
          <div className="panel p-4">
            ...
            <pre className="text-xs bg-slate-50 rounded p-2 max-h-64 overflow-auto whitespace-pre-wrap">
        ...
        {canWrite && (
          <div className="panel p-4">
            ...
            <select className="w-full mb-2 px-3 py-2 rounded-lg bg-white border border-slate-300 text-sm"
              value={reg.assetType} onChange={e => setReg({ ...reg, assetType: e.target.value })}>
              {ASSET_TYPES.map(t => <option key={t}>{t}</option>)}
            </select>
            <textarea className="w-full mb-2 px-3 py-2 rounded-lg bg-white border border-slate-300 text-sm h-28 font-mono"
              placeholder="내용 (markdown 또는 JSON)" value={reg.content} onChange={e => setReg({ ...reg, content: e.target.value })} />
            <button onClick={register} className="w-full py-2 rounded-lg bg-[#008485] text-white font-semibold text-sm">등록 (버전 히스토리 기록)</button>
...
function Generate({ canWrite }: { canWrite: boolean }) {
  ...
  return (
    <div className="max-w-2xl space-y-3">
      <textarea className="w-full px-3 py-2 rounded-lg bg-white border border-slate-300 text-sm h-24"
        value={brief} onChange={e => setBrief(e.target.value)} placeholder="디자인 브리프" />
      <select className="w-full px-3 py-2 rounded-lg bg-white border border-slate-300 text-sm"
      ...
      <div className="flex flex-wrap gap-1">
        {assets.map(a => {
          ...
          return <button key={id} onClick={() => toggle(id)}
            className={`chip text-xs ${selAssets.includes(id) ? 'text-teal-700 border-teal-400' : 'text-slate-400'}`}>{a.name}</button>;
        })}
      </div>
      <button onClick={run} className="px-5 py-2 rounded-lg bg-[#008485] text-white font-semibold text-sm">
        생성 (3개 축 variant · AgentCore Runtime)</button>
      {job && <div className="panel p-3 text-sm">
        잡 {job.job_id || job.asset_id} — 상태: <b className={job.status === 'done' ? 'text-emerald-600' : 'text-amber-700'}>{job.status}</b>
      ...
      {msg && <div className="text-[#E90061] text-sm">{msg}</div>}
    </div>
  );
}
