...
`assemble(build, browser, contract=...)` mirrors `workspace/react_runtime.py` (the build summary kept on the report)
...
`workspace.rules.contract_hash` is imported lazily, so a Runtime image without the workspace verifier modules fails
closed (`EvidenceUnavailable`) instead of assembling an unverifiable report.
"""
...
def contract_hash(contract):
    try:
        from workspace.rules import contract_hash as _hash
    except ImportError as error:                       # pragma: no cover - exercised only in a stripped image
        raise EvidenceUnavailable("contract hashing is unavailable") from error
...
def assemble(build, browser, *, contract):
    ...
    report.update(sourceHash=build["sourceHash"], bundleHash=build["bundleHash"], catalogHash=build["catalogHash"],
                  ...
                  contractHash=contract_hash(contract), outputType="react")
    return report
