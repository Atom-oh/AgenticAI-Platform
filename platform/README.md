...
이 실행들은 원본 이미지 기준이 없어 시각 비교를 수행하지 않았다. 실제 API에 연결한 새 UI의 FHD/QHD/WQHD 확인은 통과했다.
승인 후 AWS 재빌드·릴리스 검사는 진행 중이며 새 프런트엔드는 미게시다. 공개 PR 권한은 승인됐지만 PR은 아직 생성하지 않았다.
...
| AWS 릴리스 | 승인된 Astra 기준안과 Fable 시안의 같은 소스 재빌드·릴리스 검사 **진행 중**. AWS 릴리스 통과는 아직 미확정 |
| 사용자 화면 | 새 UI를 실제 API의 프로젝트·상품과 연결해 **FHD/QHD/WQHD 통과, 페이지 오류 없음** 확인. 스테이징 결과이며 새 프런트엔드는 **미게시** |
| PR | 공개 플랫폼 소스 게시 권한 승인됨. **PR 미생성**, 최종 head 검토·병합 미완료 |

이 표는 코드 `7325c72`를 기준으로 한 2026-09-11 통합 점검의 확인 범위다.
라이브 모델의 필수 검사 통과, 원본 이미지 비교 미수행, 스테이징 UI 확인, AWS 릴리스 진행, 실제 화면 게시를 구분한다.
AWS 릴리스 검사와 프런트엔드 게시·최종 PR 검토 결과는 확인 후 갱신한다.
