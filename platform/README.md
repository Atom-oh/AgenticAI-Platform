...
## 2026-09-10 — Studio 모델 선택·검수 상태 보정

`BankPlatformCore` 서버·Studio 워커·AgentCore Runtime·웹에 반영했다. 기본 모델은 유지하고
GPT-6 Astra, Claude Fable 5·5.1을 허용 목록에 추가했다. Studio의 생성·수정·리뷰, 프로세스 생성,
에이전트 빌더가 같은 목록을 사용한다. 실제 모델 호출은 기존 경계 검사를 지난다.

- Astra와 Fable 5.1의 라이브 시안 생성·저장 및 AgentCore Runtime 응답을 확인했다.
  시안 `b3c5c07765a4`와 `78f2c0263109`는 연결 검증용이며 승인하지 않았다.
- Fable 시안은 90점이어도 비필수 항목 `DEP-05` 미판정과 리뷰 응답 부분 파싱 때문에 통과하지 않았다.
  점수가 높다는 이유로 미판정 결과를 승인하는 경로를 차단한다.
- 선택한 라운드와 수정 입력·검수표를 연결한다. 새 전체 검수표는 별도 `report#<jobId>` 파티션에 저장하고,
  이력 목록 응답에는 전체 항목을 중복 싣지 않는다.
- 기존 점수·사람 승인 기록은 보존하며 새 검증 통과로 자동 승격하지 않는다. 현재 승인은 **정적 시안 승인**이다.
- 로컬 JSON·Markdown·TXT 입력, UTF-8 BOM 처리, 내용 크기 오류 안내를 추가했다.
  전체 `.fig`·ZIP·이미지 패키지 해석과 사내 React 자동 매핑은 아직 없다.
- 새 HTML의 외부 리소스 참조 검사와 CSP를 적용하고 앱의 외부 폰트 CDN을 제거했다.
  웹 업로드는 기존 생성 산출물을 삭제하지 않는다.

검증: 오프라인 테스트 516개, 프론트 타입 검사·빌드, 브라우저 모델 선택·라운드 전환·BOM 반입·인증 흐름,
라이브 모델별 생성과 저장 기록, AgentCore 기본 엔드포인트 READY/버전 6을 확인했다.
이 검증은 실제 은행 React 패키지나 실행 가능한 UX의 기능 검증을 대신하지 않는다.

배포 중 정책 보존용 스냅샷의 손실된 한글(`?`)이 DRAFT·도구 설명에 반영된 것을 발견해,
원소스와 활성 정책 4번을 기준으로 즉시 복구했다. 활성 정책 4번은 유지됐으며 DRAFT의 7개 정책 설정과
MCP 도구 설명 10개를 실제 서비스 API로 재대조했다. 정책 문자열 보존에는 CloudFormation 반환값만 사용하지 않는다.

디자이너 관점의 남은 작업과 운영 전제는 [구현·사용성 점검](../docs/14-demo/studio-implementation-review.md)을 따른다.

2026-09-11 사용자 정정: **3호망에서도 Figma 접속이 불가**하다. 외부에서 전달받은 일반 파일을 정해진 절차로 반입한다.
다음 구현의 시작점은 HTML·PNG·JPG·SVG·FIG·PDF 원본의 반입·보관과 지원 범위에 따른 미리보기다.
현재 텍스트 입력이 이 기능을 지원한다는 뜻은 아니다. [파일 반입 계약](../docs/14-demo/studio-file-intake.md)을 참고한다.

## 반응형 작업 공간

공통 콘텐츠의 1,400px 제한을 제거하고, 가용 폭에 따라 카드와 작업 패널을 재배치한다.
화면 크기를 바꿔도 선택한 모델·라운드와 입력 중인 수정 지시는 유지된다.

| CSS 뷰포트 | 시안 갤러리 | Studio 편집 |
|---|---|---|
| FHD 1920×1080 | 4열 | 설정·시안·검수 3열 |
| QHD 2560×1440 | 5열 | 설정·시안·검수 3열 |
| 와이드 3440×1440 | 7열 | 설정·시안·검수·기준표 4열 |

카드 수가 적으면 가용 공간에 맞춰 펼쳐진다. 모바일 시안의 기준 폭은 유지하고,
전체 폭 모드에서 미리보기 영역을 확장한다. 에이전트 빌더도 넓은 화면에서 편집과 채팅을 나란히 배치한다.
1440×900부터 3840×2160까지 5개 크기, 6개 화면의 24개 배치 조합에서 가로 넘침·팝업·입력 유지·미리보기 전환을 확인했다.

검수표 조회는 라운드 목록과 전체 근거를 분리한다. 긴 검수표는 요청 식별자와 JSON 이스케이프를 포함해
28,000바이트 이하로 나눠 보내고, 화면에서는 같은 버전의 조각을 모두 받은 뒤 표시한다.
도중에 기록이 변경되거나 일부만 도착하면 전체 검수표로 사용하지 않는다.
승인에는 해당 시안의 완전하고 비어 있지 않은 저장 보고서가 필요하며, 기존 HTML의 더 엄격한 CSP도 유지한다.

추가 검증: Python 521개, 검증 실행기 12개, 보고서 복원 5개 테스트와 브라우저 CSP 차단 확인.
프런트 보고서 테스트는 `cd platform/web && node --test test/reports.test.cjs`로 실행한다.
