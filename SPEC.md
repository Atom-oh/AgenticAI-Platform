...
| AWS 릴리스 | 승인된 Astra 기준안·Fable 시안의 같은 소스 재빌드·릴리스 검사 **진행 중** |
| 스테이징 UI | 실제 API의 프로젝트·상품과 연결한 새 UI가 FHD/QHD/WQHD에서 통과, 페이지 오류 없음 |
| 프런트엔드·PR | 새 프런트엔드 **미게시**, PR **미생성**. 플랫폼 소스 공개 게시 권한은 승인됐으며 게시 UI 확인·최종 head 검토·병합은 남음 |
...
진행 중인 AWS 릴리스 검사 및 프런트엔드 게시·게시 UI 검증·PR 상태는 확인 후 갱신한다.
